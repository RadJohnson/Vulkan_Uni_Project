...
const std::vector<const char*> deviceExtensions = {VK_KHR_SWAPCHAIN_EXTENSION_NAME};
    ...
    private:
        ...
        float keyInputRotate = 0;
        float keyInputTranslateX = 0;
        float keyInputTranslateZ = 0;
        float keyInputTranslateY = 0;
        ...
        void CreateInstance()
        {
            ...
            {
                throw std::runtime_error("Validation layers requested, but are unavailabe");
            }
        ...
        void CreateGraphicsPipeline()
        {
            ...
            /*std::cout << "Vertexshadersize" << vertShaderCode.size() << '\n';
            std::cout << "Fragmenthadersize" << fragShaderCode.size() << '\n';*/
        ...
        void UpdateUniformBuffer(uint32_t currentImage) 
        {
            ...
            ubo.model = glm::translate(glm::mat4(1.0f),glm::vec3(KeyboardInput().x, KeyboardInput().y, KeyboardInput().z)) * glm::rotate(glm::mat4(1.0f),/*time **/ glm::radians(KeyboardInput().w), glm::vec3(0.0f, 0.0f, 1.0f));
            ubo.view = glm::lookAt(glm::vec3(2.0f,2.0f,2.0f)/*eye position(cam I think)*/, glm::vec3(0.0f, 0.0f, 0.0f)/*center position presumably for object that will be looking at the eye pos*/, glm::vec3(0.0f, 0.0f, 1.0f)/*Up axis currently Z? == up*/);
            ubo.proj = glm::perspective(glm::radians(45.0f)/*Fov*/, swapChainExtent.width / (float)swapChainExtent.height/*aspect ratio*/, 0.1f/*near clipping plane*/, 10.0f/*far near clipping plane*/);

            ubo.proj[1][1] *= -1;//so that the object isn't rendered flipped as glm was initailly for openGl and means that the Y was flipped
        ...
        glm::vec4 KeyboardInput() 
        {
            if (glfwGetKey(window, 'W'))
            {
                keyInputTranslateX += 0.01f;
            }
            ...
            {
                keyInputTranslateY += -0.01f;
            }
            else if (glfwGetKey(window, 'D'))
            {
                keyInputTranslateY += 0.01f;
            }
            else if (glfwGetKey(window,340))
            {
                keyInputTranslateZ += -0.01f;
            }
            else if (glfwGetKey(window, 32))
            {
                keyInputTranslateZ += 0.01f;
            }
            else if (glfwGetKey(window, 'Q'))
            {
                keyInputRotate += 0.1f;
            }
            ...
            return glm::vec4(keyInputTranslateX,keyInputTranslateY,keyInputTranslateZ,keyInputRotate);
        } 
